...
class deferred_shading_volumes : public OGL::LearnOGLApp
{
public:
	...
	virtual void Render(OGL::LearnOGLContext* context) override
	{
		...
		for (uint32_t i = 0; i < mLightPositions.size(); i++)
		{
			...
			mDeferredShader->SetFloat("lights[" + std::to_string(i) + "].Linear", linear);
			mDeferredShader->SetFloat("lights[" + std::to_string(i) + "].Quadratic", quadratic);
			...
			mDeferredShader->SetFloat("lights[" + std::to_string(i) + "].Radius", radius);
		}
...
DECLARE_MAIN(deferred_shading_volumes)