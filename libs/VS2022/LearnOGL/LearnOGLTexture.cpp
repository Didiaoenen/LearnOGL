...
LearnOGLTexture::LearnOGLTexture(const std::string path, LearnOGLModel::TextureType textureType/* = LearnOGLModel::TextureType::Diffuse*/, GLenum targetType/* = GL_TEXTURE_2D*/)
	: mID(0), mPath(path), mTextureType(textureType), mTargetType(targetType)
{
	UCHAR* imageData = stbi_load(path.c_str(), &imageWidth, &imageHeight, &imageChannels, 0);
	if (!imageData)
	{
