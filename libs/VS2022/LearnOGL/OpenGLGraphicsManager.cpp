...
{
    if (OpenGLGraphicsCommonBaseManager::Initialize())
    {
        return true;
    }
...
void OGL::OpenGLGraphicsManager::BeginPass(Frame& frame)
{
    if (frame.renderToTexture)
    {
        ...
        else 
        {
            if (frame.colorTextures[1].handler) 
            {
                glFramebufferTexture(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, (GLuint)frame.colorTextures[1].handler, 0);
            }
        }
