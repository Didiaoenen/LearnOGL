...
namespace ll
{
    ...
    using UIMeshBufferMap = std::unordered_map<uint16_t, UIMeshBufferArray>;
...
private:
    ...
    std::unordered_map<std::hash_t, DescriptorSet*> _descriptorSetCache;
    ...
    std::vector<Attribute> _maskAttributes{
        Attribute{ATTR_NAME_POSITION, Format::RGB32F},
    };
